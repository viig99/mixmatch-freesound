...
parser.add_argument('--batch-size', default=3*16, type=int, metavar='N',
...
def save_checkpoint(state, is_best, val_acc, checkpoint=args.out, filename='checkpoint'):
    checkpoint_dir = os.path.join(checkpoint, "checkpoints")
    os.makedirs(checkpoint_dir, exist_ok=True)
    filepath = os.path.join(checkpoint_dir, f"{filename}_{state['epoch']}.pth.tar")
